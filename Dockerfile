...
RUN USER=root cargo new --bin app && echo 'dotenv="*"' >> /app/Cargo.toml
WORKDIR /app
...
RUN addgroup -S ifconfig && adduser -S ifconfig ifconfig && \
...
# healthcheck command
HEALTHCHECK --interval=10s --timeout=5s CMD [ "/usr/bin/curl", "--fail", "--silent", "--show-error", "http://localhost:${LISTEN_ADDR}" ]
